...
ENV UV_INSTALL_ARGS="${UV_INSTALL_ARGS}" \
  ...
  PATH="/workspace/demoapp/.venv/bin:/usr/local/bin:$PATH"
...
RUN python -m venv --copies /workspace/demoapp/.venv \
  && /workspace/demoapp/.venv/bin/pip install cython uv nodeenv  \
...
ARG LITESTAR_APP="demoapp.asgi:create_app"

## --------------------------- STANDARDIZE EXECUTION ENV ----------------------------- ##
ENV PATH="/workspace/demoapp/.venv/bin:$PATH" \
  VIRTUAL_ENV="/workspace/demoapp/.venv" \
